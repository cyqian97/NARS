...
class Robot:
        ...
        for i, event in enumerate(events):
            if event.etype == GapEventType.N:
                if event.edge.side == CCW:
                    ...
                    gap.vertex = self.vis_graph.graph.get_next_point(event.edge.p1)
                    ...
                    raise Exception(
                        f"event.edge.side must be {CCW} or {CW}, but is {event.edge.side}"
                    )
                # print(f"Gap #{gap.id} proceed")
            elif event.etype == GapEventType.P:
                if event.edge.side == CW:
                    gap_count, gap = self.find_gap(event.edge.p1)
                    gap.vertex = self.vis_graph.graph.get_prev_point(event.edge.p1)
    ...
    def gap_events(self, path_edge):
        ...
        for edge in self.vis_graph.bitcomp.get_edges():
            ...
            if (
                p and p != path_edge.p2
            ):  # path line contains the starting but not the ending point
        ...
        for edge in self.vis_graph.extlines.get_edges():
            ...
            if p and p != path_edge.p2:
                ...
                if _side == -1:
                    ...
                    raise Exception(f"ERROR: _side must be 1 or -1, but is {_side}")
